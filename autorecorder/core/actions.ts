 ...
 *
 * Its own error type because one page's silence is a break and another's is the
 * whole finding. The engine downgrades this -- and only this -- to `[ISSUE]`
 * when the page declares `knownIssue.expectsNoResponse`. Every other demo-step
 * failure (a 404, a chat surface that never renders) stays a hard failure, so
 * the escape hatch cannot be used to make a genuinely broken repo look green.
 */
