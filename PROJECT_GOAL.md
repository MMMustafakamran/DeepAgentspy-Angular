...
QA on the CopilotKit Angular DeepAgents docs
(<https://docs.copilotkit.ai/angular/deepagents>). The job is **finding bugs and
ambiguity in those doc pages**. The deliverable is `DOCUMENTED_REPORT.md` plus one
...
| `DOCUMENTED_REPORT.md` | The QA report that gets sent on, built by `ci/build-report.mjs` |

## Cycle

```
drift check → implement changed pages into frontend/+backend/ → record → report
```
