...
export function loadEnvFiles() {
  const loaded = [];
  // `backend/.env` is what `backend/langgraph.json` points the dev server at,
  // and it is the only file here that carries credentials.
  //
  // `frontend/.env.local` is a Next convention and this frontend is Angular, so
  // nothing reads it -- but it stays in the list because it costs nothing and
  // the equivalent Angular values (DEEPAGENTS_DEPLOYMENT_URL, DEEPAGENTS_GRAPH_ID
  // and PORT, all read by frontend/server.ts) are plain environment variables
  // that a developer may well park in a file rather than export by hand. Loading
  // it means the run report can state which deployment URL the runtime was
  // actually told to forward runs to -- a mismatch there produces a healthy
  // backend and an error banner in every demo, which is otherwise a long
  // afternoon.
  for (const file of [
