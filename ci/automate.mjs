...
async function main() {
  ...
  try {
    ...
    if (busy.backend) {
      ...
      //
      // --no-reload matters everywhere, and it cost a run to learn. The dev
      // server hot-reloads on any change under the repo, and this pipeline
      // writes into the repo while it runs: frontend/VERSIONS.md before
      // recording, then clips, logs and RECORD_RESULTS.json into
      // autorecorder/videos/. The watcher sees those, reloads repeatedly, and
      // eventually exits mid-suite -- which surfaces as later pages failing
      // with "the agent never replied" and no other clue.
      //
      // `--with "langgraph-cli[inmem]"` pulls the dev server in without adding
