...
interface Probe {
  ...
  detail: string;
  /**
   * Treat any HTTP response as "reachable", not just a 2xx. The DeepAgents dev
   * server answers a probing GET with whatever its own routing decides — any
   * status at all still proves the process is listening.
   */
  anyStatus?: boolean;
}
